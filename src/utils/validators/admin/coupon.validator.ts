...
const couponRules = (isUpdate = false): ValidationChain[] => [
  body('code')
    .optional(isUpdate)
    .notEmpty()
    ...
    .custom(async (code: string, { req }: Meta) => {
      ...
      if (isUpdate && coupon && (coupon as any)._id.toString() === req.params?.couponId) {
        return true;
      }
      if (coupon) {
...
export const createCouponRules = (): ValidationChain[] => couponRules();
export const updateCouponRules = (): ValidationChain[] => [
