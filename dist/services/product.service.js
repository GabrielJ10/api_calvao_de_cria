...
const listPublicProducts = async (queryParams) => {
    ...
    if (queryParams.minPrice || queryParams.maxPrice) {
        ...
        if (queryParams.minPrice)
            priceRangeQuery.$gte = parseFloat(queryParams.minPrice);
        if (queryParams.maxPrice)
            priceRangeQuery.$lte = parseFloat(queryParams.maxPrice);
    ...
    // === BUSCA NO REPOSITORY ===
    const { products, total } = await product_repository_1.default.findAllPublic(filters, options);
    // === TRANSFORMAÇÃO DOS DADOS ===
    const productsTransformed = products.map(product_transformer_1.default.transformProductForPublicList);
    // === RETORNO PADRÃO ===
    return {
...
const getPublicProductDetails = async (productId) => {
    ...
    const productObject = product_transformer_1.default.transformProductForPublicDetail(product);
    return {
        data: productObject,
