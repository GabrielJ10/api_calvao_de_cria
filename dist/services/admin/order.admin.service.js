...
const listAllOrders = async (queryParams) => {
    ...
    const { orders, total } = await order_repository_1.default.findAllAdmin(filters, options);
...
const getOrderDetails = async (orderId) => {
    const order = await order_repository_1.default.findByIdAdmin(orderId);
    if (!order) {
        throw new AppError_1.default('Pedido não encontrado.', 404);
    }
    return { data: order, message: 'Detalhes do pedido retornados com sucesso.' };
};
const updateOrder = async (orderId, updateData) => {
    const order = await order_repository_1.default.updateByIdAdmin(orderId, updateData);
    if (!order) {
        throw new AppError_1.default('Pedido não encontrado.', 404);
    }
